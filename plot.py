...
class Plot:
        ...
        for row in range(len(grid_nums_transposed)):
            for col in range(len(grid_nums_transposed[0])):
                ...
                if (self.grid[col][row].agent != None):
                    ...
                    pyplot.text(row - .3, col + .05, "Agent " + str(
                        self.grid[col][row].agent.get_id()), fontweight=500, fontsize=11)
                    ax.add_patch(circle)
            ...
            for col in range(len(grid_nums_transposed[0])):
                ...
                if (self.grid[col][row].agent != None):
                    if (self.grid[col][row].agent.get_id() == id):
                        circle = pyplot.Circle(
                            (row - .05, col + .05), radius=.3, color='red')
                        pyplot.text(row - .3, col + .05, "Agent " + str(
                            self.grid[col][row].agent.get_id()), fontweight=500, fontsize=11)
                        ax.add_patch(circle)
                    else:
                ...
                if (self.grid[col][row].obstacle == True):
                    ...
                    ax.add_patch(square)
        pyplot.figtext(.05, .95, "step = " + str(self.step), fontsize=15)
        pyplot.figtext(.4, .95, "Failed: Agent " +
                       str(id) + " stuck", fontsize=17)
        pyplot.savefig("images/step" + str(self.step) + ".png")

    def improved_visualize(self, finished_agents, agents):
        pyplot.figure(figsize=(10, 10))
        # Set up grid of coordinates with tile numbers on corresponding coordinates
        # Each tile number will have an associated color (coords with same tile number will be colored the same color for ease of visualization)
        pyplot.imshow(self.grid_nums)
    ...
    def improved_failure(self):
        ...
        pyplot.figtext(.025, .05, "step = " + str(self.step), fontsize=15)
        pyplot.figtext(.2, .92,
                       "All Agents Stuck. Begin Intermediate Target Phase.", fontsize=17)
        pyplot.savefig("images/step" + str(self.step) + ".png")

    def intermediate_target_visualize(self, finished_agents, agents, intermediate_targets):
        pyplot.figure(figsize=(10, 10))
        # Set up grid of coordinates with tile numbers on corresponding coordinates
        # Each tile number will have an associated color (coords with same tile number will be colored the same color for ease of visualization)
        pyplot.imshow(self.grid_nums)
