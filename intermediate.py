...
# Used in several methods in Intermediate class
def is_coord_in_bounds(rows, cols, coord):
    return coord[0] >= 0 and coord[0] < rows and coord[1] >= 0 and coord[1] < cols

class Intermediate:
    # Grid that the simulation is running on, passed as parameter from main.py
    grid = []
    # List of all agents remaining in simulation passed as parameter from main.py
    agents = []
    # Goal coord of agents passed as parameter from main.py
    goal_coords = []
    # Current time step of simulation, passed as parameter from main.py
    step = 0
    # Maximum number of steps allowed for simulation, passed as parameter from main.py
    max_num_steps = 0
    # List of agents that reached goal, passed as parameter from main.py
    finished_agents = []
    ...
    # Returns time step of simulation
    def get_step(self):
        return self.step

    # Returns list of finished agents
    ...
    # If there is a path, add that coordinate to intermediate_coordinates
    ...
    # Given an id as a parameter, loop through agents list and find agent with that id
    # Return the agent as well as the index of the agent (in tuple format) if agent is found
    # Return -1 and index variable as tuple if agent not found
    ...
    # Sets up intermediate_candidates_2 as list of all possible intermediate targets in the grid
    def create_agent_intermediate_goal_mapping(self):
        ...
        # Add remaining intermediate targets to intermediate_candidates_2 list to make a comprehensive list
        for coord in self.intermediate_candidates:
